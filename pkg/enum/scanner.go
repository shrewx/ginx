...
type EnumScanner struct {
	...
	enums map[*types.TypeName]Values
}

func NewEnumScanner(pkg *packagesx.Package) *EnumScanner {
	return &EnumScanner{
		pkg:   pkg,
		enums: make(map[*types.TypeName]Values, 0),
	}
}

func (e *EnumScanner) Enum(typeName *types.TypeName) Values {
	if typeName == nil {
		return nil
	}
...
func sortEnumValues(enumValues Values) Values {
