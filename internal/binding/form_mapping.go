...
import (
	...
	"github.com/shrewx/ginx/internal/byteconv"
	"maps"
	"mime/multipart"
	"reflect"
	"strconv"
...
// SetOptions 设置选项，用于解析标签中的 default 值等
// 这是 setOptions 的导出版本，供外部使用
type SetOptions struct {
	setOptions
}
...
// ParseSetOptions 从 structField 的 name 标签中解析 SetOptions
// 支持解析 default=value 选项，如 name:"param,default=123"
func ParseSetOptions(structField reflect.StructField) SetOptions {
	var opt SetOptions
	nameTag := structField.Tag.Get("name")
	...
	var optStr string
	for len(opts) > 0 {
		optStr, opts = head(opts, ",")
		if k, v := head(optStr, "="); k == "default" {
...
// SetFieldByForm 直接绑定单个字段，使用 form 数据源
// value: 字段的 reflect.Value
// field: 字段的 reflect.StructField（用于标签解析，如 time_format）
// form: 数据源 map[string][]string
// tagValue: 参数名（用于从 form 中查找值）
// opt: 设置选项（如默认值）
func SetFieldByForm(value reflect.Value, field reflect.StructField, form map[string][]string, tagValue string, opt SetOptions) (isSet bool, err error) {
	// 转换 SetOptions 为内部 setOptions
	internalOpt := setOptions{
		isDefaultExists: opt.isDefaultExists,
		defaultValue:    opt.defaultValue,
	}
	return setByForm(value, field, form, tagValue, internalOpt)
}
